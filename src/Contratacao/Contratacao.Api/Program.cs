...
builder.Host.UseSerilog((context, loggerConfiguration) => loggerConfiguration
    ...
    .Enrich.WithProperty("Service", "Contratacao.Api"));

builder.Services
    .AddContratacaoInfrastructure(builder.Configuration)
...
{
    options.AddPolicy(AuthorizationPolicies.ContratacaoCriar, policy =>
...
builder.Services
    .AddHealthChecks()
    .AddNpgSql(
        builder.Configuration.GetConnectionString("ContratacaoDb")!,
        name: "postgres",
        tags: ["ready"])
    .AddCheck<RabbitMqHealthCheck>("rabbitmq", tags: ["ready"]);

builder.Services.AddOpenTelemetry()
    .ConfigureResource(resource => resource.AddService("Contratacao.Api"))
    .WithTracing(tracing => tracing
        .AddAspNetCoreInstrumentation()
        .AddHttpClientInstrumentation() // also traces the outgoing call to Proposta.Api
        .AddOtlpExporter())
...
// Em produção, migrations são aplicadas por um pipeline/job dedicado (Contratacao.Migrator),
...
if (app.Environment.IsDevelopment() && app.Configuration.GetValue<bool>("RunMigrationsOnStartup"))
{
    ...
    var dbContext = migrationScope.ServiceProvider.GetRequiredService<Contratacao.Infrastructure.Persistence.ContratacaoDbContext>();
