...
using Proposta.Application.DependencyInjection;
...
builder.Host.UseSerilog((context, loggerConfiguration) => loggerConfiguration
    ...
    .Enrich.WithProperty("Service", "Proposta.Api"));

builder.Services
    .AddPropostaInfrastructure(builder.Configuration)
...
{
    options.AddPolicy(AuthorizationPolicies.PropostaCriar, policy =>
        policy.RequireRole("usuario", "Usuario", "analista", "Analista", "admin", "Admin"));

    options.AddPolicy(AuthorizationPolicies.PropostaGerenciar, policy =>
        policy.RequireRole("analista", "Analista", "admin", "Admin"));
});
...
builder.Services
    .AddHealthChecks()
    .AddNpgSql(
        builder.Configuration.GetConnectionString("PropostaDb")!,
        name: "postgres",
        tags: ["ready"])
    .AddCheck<RabbitMqHealthCheck>("rabbitmq", tags: ["ready"]);

builder.Services.AddOpenTelemetry()
    .ConfigureResource(resource => resource.AddService("Proposta.Api"))
...
// Em produção, migrations são aplicadas por um pipeline/job dedicado (Proposta.Migrator),
...
if (app.Environment.IsDevelopment() && app.Configuration.GetValue<bool>("RunMigrationsOnStartup"))
{
    ...
    var dbContext = migrationScope.ServiceProvider.GetRequiredService<Proposta.Infrastructure.Persistence.PropostaDbContext>();
